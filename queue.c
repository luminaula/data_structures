...
queue_t *queue_create(size_t elem_size) {
    ...
    queue->read_chunk = queue_new_chunk(queue->chunk_size, NULL);
...
void queue_free(queue_t *queue) {
    ...
    while (queue->read_chunk) {
        void *next = queue_get_next_chunk(queue->chunk_size, queue->read_chunk);
...
void queue_push(queue_t *queue, void *data) {
    ...
    if (write_offset + queue->elem_size > queue->chunk_size) {
        queue->write_chunk = queue_new_chunk(queue->chunk_size, queue->write_chunk);
...
void *queue_pop(queue_t *queue) {
    ...
    if (read_offset + queue->elem_size > queue->chunk_size) {
        ...
        queue->read_chunk =  queue_get_next_chunk(queue->chunk_size, queue->read_chunk);
        queue->read_head = queue->read_chunk;
        queue->chunk_count--;
        free(tmp);
    }
